...
//   console.log('recentsession', sessions)
//   return (
...
//                             <Code2 className="w-6 h-6 text-white"></Code2>
//                           </div>
          ...
          {isLoading ?
            (
              <div className="col-span-full flex items-center justify-center py-20">
                <Loader className="w-10 h-10 animate-spin text-primary"></Loader>
              </div>
            ) :
            (
              <div className='flex flex-row'>
                          {sessions.length > 0 ? sessions.map((session) => (
               <div>
                            <div key={session._id} className={` card relative ${session.status === 'active' ? 'bg-success/10 border-success/30 hover:border-success/60' : 'bg-base-200 border-base-300 hover:border-primary/30'}`}>
                    {
                  
                                }
                           
                    <div className="card-body w-64 p-5 flex" >
                  ...
                  <div className="col-span-full text-center py-16">
                    <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-accent/20 to-secondary/20 rounded-3xl flex items-center justify-center">
                      <Trophy className="w-10 h-10 text-accent/50"></Trophy>
                    </div>
                    <p className="text-lg font-semibold opacity-70 mb-1">no sessions yet</p>
                    <p className="text-sm opacity-50">start your learning journey today</p>
                  </div>
                )
                }
              </div>)}
