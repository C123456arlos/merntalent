...
// import { useStreamClientData } from "../hooks/useSessions" 
// const SessionPage = ({
//     loadingSession,
...
//     const { call, channel, chatClient, isInitializingCall, streamClient } = useStreamClient({session,
//        loadingSession,
//         isHost, isParticipant})
                                        ...
                                        <div className="flex items-start justify-between mb-3">
                                            <div>
                                                ...
                                                <p className="text-base-content/60 mt-2">
                                                    host: {session?.host?.name || 'loading'} &middot; {' '}
                                                ...
                                                {isHost && session?.status === 'active' && (
                                                    <button onClick={handleEndSession}
                                                        ...
                                                        className="btn btn-error btn-sm gap-2">{
                                                            endSessionMutation.isPending ? (<Loader2Icon className="w-4 h-4 animate-spin"></Loader2Icon>) : (<LogOutIcon className="w-4 h-4"></LogOutIcon>)
                                                        }end session</button>
                                                )}
                                                    ...
                                                    {problemData.examples.map((example, idx) => (
                                                        <div key={idx}>
                                                            <div className="flex items-center gap-2 mb-2">
                                                                ...
                                                                <p className="font-semibold text-base-content">exmaple {idx + 1}</p>
                                                            </div>
                                                            <div className="bg-base-200 rounded-lg p-4 font-mono text-sm space-y-1.5">
                                                                ...
                                                                {example.explanation && (
                                                                    <div className="pt-2 border-t borde-base-300 mt-2">
                                ...
                                <Panel defaultSize={50} minSize={20}>
                                    <PanelGroup direction="vertical">
                                        <Panel defaultSize={70} minSize={30}>
                                            <StudioEditorPanel selectedLanguage={selectedLanguage}
                                            code={code} isRunning={isRunning} onLanguageChange={handleLanguageChange}onCodeChange={(value)=>setCode(value)} onRunCode={handleRunCode}></StudioEditorPanel>
                                        </Panel>
                                    ...
                                    <div className="card bg-base-100 shadow-xl max-w-md">
                                        <div className="card-body items-center text-center">
                                            <div className="w-24 h-24 bg-error/10 rounded-full flex items-center justify-center mb-4">
                                                <PhoneOffIcon className="w-12 h-12 text-error"></PhoneOffIcon>
                                            </div>
                                        ...
                                        <div className="h-full">
                                            <StreamVideo client={streamClient}>
                                                <StreamCall call={call}>
                                                    <VideoCallUI chatClient={chatClient} channel={channel}></VideoCallUI>
                                                </StreamCall>
                                            </StreamVideo>
                                        </div>
                            )}
